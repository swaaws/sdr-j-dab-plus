...
const	int	EXTIO_BASE_TYPE_SIZE = sizeof (float);
	...
	theConverter (2500000, 2048000, 23768) {
int	result;
...
DSPCOMPLEX outV [theConverter. getOutputsize ()];
int32_t amount, i;

	for (i = 0; i < nSamples; i ++)
	   if (theConverter. convert (DSPCOMPLEX (sbuf [2 * i],
	                              sbuf [2 * i + 1]), outV, &amount)) 
	      theBuffer	-> putDataIntoBuffer (outV, amount);
	return 0;
}
