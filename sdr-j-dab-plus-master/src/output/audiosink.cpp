...
int32_t	i;
	...
	if ((size & (size - 1)) != 0)
	   ...
	   this	-> size		= size;
	this -> Latency		= 2;
	_O_Buffer		= new RingBuffer<float>(2 * size);
	portAudio		= false;
	writerRunning		= false;
	...
	ostream		= NULL;
	dumpFile	= NULL;
	upFilter	= new LowPassFIR (5, 12000, rate);
}
...
//
//	for te DAB receiver, the output is in "int16_t" s, and the rates
//	are either 48000 or 24000.
...
int32_t	available;
	...
	for (i = 0; i < amount; i ++) {
	   tmp 	= DSPCOMPLEX ((float (V [2 * i])) / 32767.0,
	                     (float (V [2 * i + 1]) / 32767.0));
	   tmp	= upFilter -> Pass (tmp);
	   buffer [4 * i]	= real (tmp);
	   buffer [4 * i + 1]	= imag (tmp);
	   tmp	= upFilter  -> Pass (DSPCOMPLEX (0, 0));
	   buffer [4 * i + 2]	= real (tmp);
	   buffer [4 * i + 3]	= real (tmp);
	}

	if (dumpFile != NULL)
	   sf_writef_float (dumpFile, buffer, 2 * amount);
	available = _O_Buffer -> GetRingBufferWriteAvailable () & ~01;
	_O_Buffer	-> putDataIntoBuffer (buffer,
	                                      minimum (4 * amount, available));
}
//
...
void	audioSink::audioOut	(int16_t *V, int32_t amount) {
